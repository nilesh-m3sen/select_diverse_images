...
# Parallel image feature extraction
...
with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
    ...
    for batch_images, batch_paths in tqdm(dataloader, desc="Extracting Features"):
        futures.append(executor.submit(extract_features_batch, batch_images))
...
# Parallel Saving of Selected Images
...
with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
    list(tqdm(executor.map(save_image, final_subset), total=len(final_subset), desc="Saving Selected Images"))
